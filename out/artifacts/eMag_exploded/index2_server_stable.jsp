...
<head>
  ...
  //----
        ...
        if(!status.equals("editing")){
            ...
            try {
                for(int i=0; i< allEvents.size(); i++)
                {
                      ...
                      out.println("events_image["+i+"] =\"resources/"+ StringEscapeUtils.escapeJavaScript(singleEvent.getImagePath()) + " \" ;");
                      ...
                      out.println("events_link_kannada["+i+"] =\"resources/"+ singleEvent.getLink_kannada() + " \" ;");
                }
            }catch(Exception e){
          ...
          List<NewsFeed> listOfAllNews = news.getNewsFeed(pageToDisplay, year, monthName, status);
                              ...
                              for(int i=0; i< listOfAllNews.size(); i++){
                                  ...
                                  if(extension != null && extension.length() > 2)
                                  {
                                      ...
                                      {
                                          out.print("news_link["+i+"]=\"resources/"+StringEscapeUtils.escapeJavaScript(singleNews.getLink())+"\" ;");
                                          out.print("news_link_kannada["+i+"]=\"resources/"+StringEscapeUtils.escapeJavaScript(singleNews.getLinkKannada())+"\" ;");
                                      }else{
          ...
          List<Stories> allStories = story.getAllStories(pageToDisplay, monthName, year, 3, status);
                          ...
                          for(int i=0; i<allStories.size(); i++)
                          {
                                  ...
                                  out.print("stories_image["+i+"]=\"resources/"+ StringEscapeUtils.escapeJavaScript(singleStory.getImagePath()) +"\" ;");
                                  out.print("stories_link_kannada["+i+"]=\"resources/"+ StringEscapeUtils.escapeJavaScript(singleStory.getLinkKannada()) +"\" ;");
                           ...
                           for(int i=0; i<allRecentArticles.size(); i++)
                           {
                                  ...
                                  if(extension.length() > 2)
                                  {
                                     extension = article.getLink().substring(article.getLink().lastIndexOf('.'), article.getLink().length()).toLowerCase();
                                      if(extension.equals(".jpg") || extension.equals(".jpeg") || extension.equals(".png") || extension.equals(".pdf"))
                                      {
                                          out.print("recent_article_link["+i+"] = \"resources/"+ article.getLink() +"\";");
                                          out.print("recent_article_link_kannada["+i+"] = \"resources/"+ article.getLinkKannada() +"\";");
                                      }else{
            ...
            List<Miscellaneous> allMiscellaneous = rows.getAllMiscellaneous(pageToDisplay, monthName, year, status);
                          for(int i=0; i<allMiscellaneous.size(); i++)
                          {
                                  ...
                                  if(extension.length() > 2)
                                  {
                                     extension = row.getLink().substring(row.getLink().lastIndexOf('.'), row.getLink().length()).toLowerCase();
                                      if(extension.equals(".jpg") || extension.equals(".jpeg") || extension.equals(".png") || extension.equals(".pdf")){
                                          out.print("miscellaneous_link["+i+"]=\"resources/"+row.getLink()+"\";");
                                          out.print("miscellaneous_link_kannada["+i+"]=\"resources/"+row.getLinkKannada()+"\";");
                                      }else
                                          {
              ...
              List<Advertisements> allAdds = addRows.getAllAdvertisement();
                  for(int i=0; i<allAdds.size(); i++)
                  {
                      ...
                      out.print("advertisement_image["+i+"]=\"resources/"+StringEscapeUtils.escapeJavaScript(singleAdd.getImagePath())+"\";");
    ...
    <!--fonts-->
    <link rel="stylesheet" id="custom-google-fonts-css" href="http://fonts.googleapis.com/css?family=Lato%7CRoboto+Condensed%7CRoboto%7CKarla%3A300%2C400%2C600%2C700&amp;ver=4.5.3" type="text/css" media="all">
    <style>
        html, body {
            width: 100%;
